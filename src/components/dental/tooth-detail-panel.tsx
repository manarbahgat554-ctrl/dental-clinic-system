...
export function ToothDetailPanel({ patientId, toothNumber, onClose }: ToothDetailPanelProps) {
  ...
  const currentRecord = toothNumber ? teeth.find((t: ToothRecord) => t.toothNumber === toothNumber) : undefined;
  ...
  useEffect(() => {
    if (currentRecord) {
      ...
      setClinicalNotes(currentRecord.clinicalNotes || '');
    } else {
      setStatus('healthy');
      setSurfaces({});
      setDiagnosis('');
  ...
  const saveMutation = useMutation({
    mutationFn: (input: Partial<ToothRecord> & { patientId: string; toothNumber: number }) =>
