...
export function ImageViewer({ images, index, onClose, onNavigate }: ImageViewerProps) {
  ...
  const previewable = isPreviewable(current.fileExt || '');
  const isPdf = current.fileExt === 'pdf';
  const is3d = current.fileExt === 'stl';
  const isDicom = current.fileExt === 'dcm' || current.fileExt === 'dicom';
        ...
        >
          ...
          >
            <div className="min-w-0 flex-1">
              <p className="truncate font-medium">{current.imageName}</p>
              <div className="mt-0.5 flex items-center gap-2 text-xs text-white/70">
                <Badge variant="secondary">{current.imageType}</Badge>
                <span>{formatDate(current.createdAt)}</span>
                {current.toothNumber && <span>· Tooth {current.toothNumber}</span>}
              </div>
            </div>
            <div className="flex items-center gap-1.5">
              <a href={current.imageUrl} download={current.imageName}>
            ...
            {previewable ? (
              ...
              <div className="flex flex-col items-center gap-4 text-white/80">
                ...
                <div className="text-center">
                  <p className="text-lg font-medium">{current.imageName}</p>
          ...
          {images.length > 1 && (
            ...
            >
              {images.slice(Math.max(0, index! - 4), index! + 5).map((img, i) => {
                ...
                const thumbPreviewable = isPreviewable(img.fileExt || '');
                return (
                  <button
                    key={img.id}
