...
export function ImageGallery({ patientId, patientName }: ImageGalleryProps) {
  ...
  mutationFn: async (image: RadiologyImage) => {
    console.log("STEP 1 - Start Delete");

    console.log("STEP 2 - Delete Storage");
    await deleteRadiologyFile(image.storage_path);
    console.log("STEP 3 - Storage Deleted");

    console.log("STEP 4 - Delete Database");
    await queries.radiology.remove(image.id);
    console.log("STEP 5 - Database Deleted");
  },
  ...
  const filtered = filter === 'all' ? images : images.filter((img) => img.image_type === filter);

  const categoryCounts = IMAGE_TYPES.reduce((acc, type) => {
    acc[type] = images.filter((img) => img.image_type === type).length;
    return acc;
      ...
      <AlertDialog open={!!deleteTarget} onOpenChange={(o) => !o && setDeleteTarget(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This will permanently remove "{deleteTarget?.image_name}" from both storage and the database. This action cannot be undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
