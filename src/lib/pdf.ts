...
export function generateInvoicePDF(invoice: Invoice, patient?: Patient) {
  ...
  doc.setFontSize(10);
  doc.setFont('helvetica', 'normal');
  doc.text(`Invoice #: ${invoice.invoice_number}`, 14, 53);
  doc.text(`Date: ${formatDate(invoice.created_at)}`, 14, 59);
  if (invoice.due_date) doc.text(`Due: ${formatDate(invoice.due_date)}`, 14, 65);

  // Patient info
  if (patient) {
    doc.text('Bill To:', pageWidth - 14, 45, { align: 'right' });
    doc.setFont('helvetica', 'bold');
    doc.text(`${patient.first_name} ${patient.last_name}`, pageWidth - 14, 51, { align: 'right' });
    doc.setFont('helvetica', 'normal');
  ...
  doc.text(`Tax (${invoice.tax_rate}%): ${formatCurrency(invoice.tax_amount)}`, pageWidth - 14, finalY + 6, { align: 'right' });
  ...
  doc.setFont('helvetica', 'normal');
  doc.setFontSize(10);
  doc.text(`Paid: ${formatCurrency(invoice.paid_amount)}`, pageWidth - 14, finalY + 26, { align: 'right' });
  doc.setFont('helvetica', 'bold');
  doc.text(`Balance: ${formatCurrency(invoice.total - invoice.paid_amount)}`, pageWidth - 14, finalY + 32, { align: 'right' });
