...
export function AppointmentsPage() {
  ...
  const { data: appointments = [], isLoading } = useQuery({
    queryKey: queryKeys.appointmentsByRange(range),
        ...
        <CardContent className="p-4">
          {isLoading ? (
            ...
            <MonthView
              ...
              onAppointmentClick={(apt) => setSelectedDate(new Date(apt.startTime))}
            />
          ) : view === 'week' ? (
            <WeekView
              currentDate={currentDate}
        ...
        <DialogContent className="sm:max-w-md">
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <Input id="treatmentType" placeholder="e.g. Cleaning, Filling" {...register('treatmentType')} />
            </div>
            <div className="space-y-2">
            ...
            >
              ...
              <div className="mt-1 space-y-0.5">
                {dayAppts.slice(0, 3).map((apt) => (
                  ...
                  >
                    {formatTime(apt.startTime)} {apt.patient ? `${apt.patient.firstName}` : ''}
                  </div>
                ))}
                {dayAppts.length > 3 && (
          ...
          return (
            ...
            >
              <div className="flex w-20 flex-col items-center">
                <span className="text-sm font-bold">{formatTime(apt.startTime)}</span>
                <span className="text-xs text-muted-foreground">{apt.durationMin}min</span>
              </div>
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="text-xs text-muted-foreground">{apt.treatmentType || apt.title || apt.chair}</p>
              </div>
              <select
                value={apt.status}
